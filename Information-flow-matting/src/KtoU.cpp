...
SparseMatrix<double> lle_KtoU(my_vector_of_vectors_t& indm, 
	my_vector_of_vectors_t& fv_unk, my_vector_of_vectors_t& fv_fg, 
	my_vector_of_vectors_t& fv_bg, float eps, Mat& wf){
	...
	for(int i = 0; i < n; i++){
		...
		for(int j = 0; j < k/2; j++){
			...
			// nbr_ind = indm[i][j];
			// triplets.push_back(T(ind[{i,0}], ind[{nbr_ind, 1}], weights.at<float>(j,0)));
		}

		for(int j = k/2; j < k; j++){
			weights.at<float>(j,0) /= sum;
			// nbr_ind = indm[i][j];
			// triplets.push_back(T(ind[{i,0}], ind[{nbr_ind, 2}], weights.at<float>(j,0)));
		}
